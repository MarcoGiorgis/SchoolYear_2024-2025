...
def login():
    ...
    if request.method == 'POST':
        ...
        if userDb and userDb[2] == hash_password(password):
            if userDb[3] == "adm":
                resp = make_response(redirect(url_for('homeADM')))
            else:
                resp = make_response(redirect(url_for('homeUSR')))
            resp.set_cookie('username', username, max_age=60)
            return resp
        
    return render_template('login.html')

@app.route('/homeADM', methods=['GET', 'POST'])
def homeADM():
    resp = None
    if request.method == 'POST':
...
@app.route('/homeUSR', methods=['GET', 'POST'])
def homeUSR():
    saldo = 0
    resto = 0
    if request.method == 'POST':
...
def index():
    ...
    if username:
        ...
        with conn:
            cur = conn.cursor()
            cur.execute("SELECT tipo FROM users WHERE username = ?", (username,))
